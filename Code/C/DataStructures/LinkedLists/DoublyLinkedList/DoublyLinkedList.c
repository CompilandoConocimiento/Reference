...
void InsertSort(List **ListX, Item *ItemX, int Mode){           // ==== INSERT IN ORDER OF THE LIST =====
    if (ItemX == NULL) return;                                  //very funny :/ 
    if (Head == NULL) {InsertAtHead(ListX, ItemX); return;}     //Upps, special case

    if (CompareItems(ItemX,(Head->NodeItem), Mode) < 1)         //Special case where it dont work
    ...
    Node *CurrentNode = Head;                                   //Make it a counter
    while (CompareItems(ItemX,(NextNode->NodeItem),Mode)==1){   //While we didn't have found the pointer
...
Item* EliminateItem(List **ListX, Item *ItemX, int Mode){       // ==== ELIMINATE AN ITEM OF THE LIST  ==
    if (Head == NULL || ItemX == NULL) return NULL;             //Funny, very funny :/             

    if (CompareItems((Head->NodeItem),ItemX, Mode) == 0)        //Special case where it dont work
        return EliminateAtHead(ListX);                          //So call something that do it

    Node *CurrentNode = Head;                                   //Make it a counter
    while (CompareItems((NextNode->NodeItem),ItemX,Mode)!=0){   //While we didn't have found the pointer
        GoNextNode;                                             //Go next node please
        if (NextNode == NULL) return NULL;                      //If we are at the end the list 
    }
...
Item* PeekItem(List **ListX, Item *ItemX, int Mode){            // ==== PEEK AN ITEM OF THE LIST  ==
    if (Head == NULL || ItemX == NULL) return NULL;             //Funny, very funny :/

    Node *CurrentNode = Head;                                   //Make it a counter
    while(CompareItems((CurrentNode->NodeItem),ItemX,Mode)!=0){ //Lets travel until the n element
...
void ShowList(List **ListX){                                    // ==== SHOW ALL THE ELEMENTS OF A LIST =====
    ...
    for (; CurrentNode != NULL; GoPrevNode)                     //No in reverse
        ShowItem(CurrentNode->NodeItem);                        //And for each Node, show me the info inside
}
