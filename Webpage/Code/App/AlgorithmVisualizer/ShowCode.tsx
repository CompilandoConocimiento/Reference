  ...
  render() {
    ...
    return (
      <ConfigContext.Consumer>
        {Config => (
          <ShowCode ID={this.props.fileName + this.props.partOfFile} Text={Text} Config={Config} />
        )}
      </ConfigContext.Consumer>
    )
  }
...
export function loadTheme(theme: string) {
  ...
  themeLink[
    "href"
  ] = `https://cdnjs.cloudflare.com/ajax/libs/highlight.js/9.12.0/styles/${theme}.min.css`
  window["theme"] = theme
}
...
const ShowCode: React.FunctionComponent<ShowCodeProps> = props => {
  ...
  return (
    <pre id={props.ID} style={PreStyle} {...CardProps}>
      <code style={CodeStyle}>{props.Text.join("\n")}</code>
    </pre>
  )
}
